...
describe('serialize()', () => {
  ...
  test('serializes arrays', () => {
    ...
    expect(serialize([[]])).toBe('[[]]');
    expect(serialize([new Error('test')]))
      .toContain('[{"name":"Error","message":"test","stack":"Error: test');
  });
  ...
  test('serializes Errors', () => {
    ...
    //      for test stability.
    expect(serialize(e))
      .toContain('{"name":"Error","message":"oh crap","stack":"Error: oh crap');
  ...
  test('serializes HTMLElements', () => {
    ...
    expect(serialize(div))
      .toBe('<div id="ID" class="CLASS1 CLASS2" data-other="OTHER">');
    expect(serialize([div]))
      .toBe('["<div id=\\"ID\\" class=\\"CLASS1 CLASS2\\" data-other=\\"OTHER\\">"]');
  });
