  ...
  static createTelemetryFromRequest(request: http.ClientRequest): NetworkTelemetry {
    ...
    request.once('response', (response: http.IncomingMessage) => {
      ...
      if (networkTelemetry.statusCode >= 400) {
        AgentRegistrar.getCurrentAgent(request['domain'])
          .captureError(new Error(`${networkTelemetry.statusCode} ${networkTelemetry.statusText}: ${networkTelemetry.method} ${networkTelemetry.url}`));
      }
    });
